...
def prepare_dim_reduction(
    ...
    if basis is None:
                ...
                if layer is None:
                    ...
                    if genes is not None:
                        ...
                        CM = (
                            adata.layers[layer]
                            if "use_for_dynamics" not in adata.var.keys()
                            else adata[:, adata.var.use_for_dynamics].layers[
                                layer
                            ]
                        )
        ...
        if pca_key in adata.obsm.keys():
            ...
            CM = CM[:, valid_ind]
            adata, fit, _ = pca(
                adata, CM, n_pca_components=n_pca_components, pca_key=pca_key
            )
            adata.uns[
                "explained_variance_ratio_"
            ] = fit.explained_variance_ratio_[1:]
...
def run_reduce_dim(
    ...
    if reduction_method == "trimap":
        ...
        layer = (
            neighbor_key.split("_")[0]
            if neighbor_key.__contains__("_")
            else None
        )
        conn_key = (
