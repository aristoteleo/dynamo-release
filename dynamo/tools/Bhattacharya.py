    ...
    for i in np.arange(x_lim[0], x_lim[1] + xyGridSpacing, xyGridSpacing):
        for j in np.arange(y_lim[0], y_lim[1] + xyGridSpacing, xyGridSpacing):
            ...
            if (path_counter == 0):
                ...
                attractors_num_X_Y = np.vstack((attractors_num_X_Y, current_att_num_X_Y)) if attractors_num_X_Y is not \
                                                                                             None else np.array(
                    [current_att_num_X_Y])  # append array (vertically)
                attractors_pot = np.vstack((attractors_pot, Pot)) if attractors_pot is not \
                                                                     None else np.array(
                    [Pot])  # append attractor potentials to array (vertically)
                ...
                if (endPt_dist_sqr > (2 * (xyGridSpacing ** 2))):
                    ...
                    if new_attr_found == 1:
                        ...
                        attractors_num_X_Y = np.vstack(
                            (attractors_num_X_Y, current_att_num_X_Y))  # append array (vertically)
                        ...
                        attractors_pot = np.vstack(
                            (attractors_pot, Pot))  # append attractor potentials to array (vertically)
                        ...
                        if (startPt_dist_sqr < (2 * (xyGridSpacing ** 2))):
                            curr_sepx = [path_tag[path_counter - 1], path_tag[path_counter],
                                         (path_counter - 1)]  # create array
                            sepx_old_new_pathNum = np.vstack(
                                (sepx_old_new_pathNum, curr_sepx)) if sepx_old_new_pathNum is not \
                                                                      None else np.array(
                                [curr_sepx])  # append array (vertically)
...
def alignment(numPaths, numTimeSteps, pot_path, path_tag, attractors_pot, x_path, y_path, grid=100,
    ...
    Zgrid = griddata(np.hstack((x_p_list, y_p_list)), pot_p_list, np.vstack((Xgrid.flatten(), Ygrid.flatten())).T,
