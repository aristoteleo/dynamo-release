...
def moran_i(adata, X_data=None, genes=None, layer=None, weighted=True, assumption="permutation", local_moran=False):
...
def two_groups_degs(
    ...
    test_cells, control_cells = adata.obs[group] == test_group, adata.obs[group].isin(control_groups)
    ...
    de = pd.DataFrame(
        de, columns=["gene", "versus_group", "exp_frac", "rbc", "log2_fc", "pval", "specificity", "diff_ratio_pos"]
    )
...
def diff_test_helper(
    ...
    poisson_df["AUX_OLS_DEP"] = poisson_df.apply(
        lambda x: ((x["expression"] - x["mu"]) ** 2 - x["expression"]) / x["mu"], axis=1
    )
