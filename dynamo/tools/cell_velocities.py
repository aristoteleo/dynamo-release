    ...
    **kmc_kwargs
    ...
    ---------
        ...
        calc_rnd_vel: `bool` (default: `False`)
            X logic flag to determine whether we will calculate the random velocity vectors which can be plotted downstream
            as a negative control and used to adjust the quiver scale of the velocity field.
        xy_grid_nums: `tuple` (default: `(50, 50)`).
            X tuple of number of grids on each dimension.
    ...
    if method == "kmc":
        ...
        raise NotImplementedError(f'{method} method is not implemented for cell_velocity yet.')
...
def permute_rows_nsign(A):
    ...
    ----------
        A: `np.array`
            X numpy array that will be permuted.
