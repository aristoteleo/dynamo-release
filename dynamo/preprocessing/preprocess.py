...
def estimateSizeFactor(adata, locfunc = np.mean, round_exprs = True, method = 'mean-geometric-mean-total'):
...
def estimateDispersion(adata, modelFormulaStr="~ 1", relative_expr=True, min_cells_detected=1, removeOutliers=True):
    import re
...
def parametricDispersionFit(disp_table, initial_coefs=np.array([1e-6, 1])):
    ...
    while True:
        ...
        if (coefs[0] < initial_coefs[0]):
            ...
            raise ( "Parametric dispersion fit failed. Try a local fit and/or a pooled estimation. (See '?estimateDispersions')" )
...
def partitionCells(adata):
    return adata


def learnGraph(adata):
    return adata


def recipe_monocle(adata):
    preprocessAdata(adata)
...
def recipe_monocle(adata, method = 'PCA', num_dim = 50, norm_method = 'log', residualModelFormulaStr = None, pseudo_expr = 1, relative_expr = True, scaling = True, **kwargs):
    adata = estimateSizeFactor(adata)
