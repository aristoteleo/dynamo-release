...
def Gillespie(a=None, b=None, la=None, aa=None, ai=None, si=None, be=None, ga=None, C0=np.zeros((5, 1)), t_span=[0, 50],
              n_traj=1, t_eval=None, dt=1, method='basic', verbose=False):
    ...
    if method == 'basic':
        ...
        trajs_C = simulate_multigene(a=None, b=None, la=None, aa=None, ai=None, si=None, be=None, ga=None, C0=np.zeros((5, 1)), t_span=[0, 50], n_traj=1, t_eval=None,
        ...
        a_ut = 1.0 * r / tau
        a_t = 0.5 * r / tau
        params_untreat_unlab = {'a1': a_ut, 'b1': b, 'c1': c, 'a2': a_ut, 'b2': b, 'c2': c,
                                'a1_l': 0, 'b1_l': 0, 'c1_l': 0, 'a2_l': 0, 'b2_l': 0, 'c2_l': 0, 'K': K, 'n': n,
                                'be1': beta, 'ga1': gamma, 'et1': eta, 'de1': delta,
                                'be2': beta, 'ga2': gamma, 'et2': eta, 'de2': delta}
        params_treat_unlab = {'a1': a_t, 'b1': b, 'c1': c, 'a2': a_t, 'b2': b, 'c2': c,
                              'a1_l': 0, 'b1_l': 0, 'c1_l': 0, 'a2_l': 0, 'b2_l': 0, 'c2_l': 0, 'K': K, 'n': n,
                              'be1': beta, 'ga1': gamma, 'et1': eta, 'de1': delta,
                              'be2': beta, 'ga2': gamma, 'et2': eta, 'de2': delta}
        params_treat_lab = {'a1': 0, 'b1': 0, 'c1': 0, 'a2': 0, 'b2': 0, 'c2': 0,
                            'a1_l': a_t, 'b1_l': b, 'c1_l': c, 'a2_l': a_t, 'b2_l': b, 'c2_l': c, 'K': K, 'n': n,
                            'be1': beta, 'ga1': gamma, 'et1': eta, 'de1': delta,
                            'be2': beta, 'ga2': gamma, 'et2': eta, 'de2': delta}
        ...
        kin_cell_ids, kin_Trajectory, kin_Step = ['kin_traj_%d_time_%f' % (i, j) for j in kin_Tl for i in range(n_cell)], \
        ...
        one_shot_cell_ids, one_shot_Trajectory, one_shot_Step = ['one_shot_traj_%d_time_%d' % (i, j) for j in kin_T_CP for i in range(n_cell)], \
                                                 ...
                                                 [j for j in kin_T_CP for i in range(n_cell)]  # first n_traj and then steps
        begin_cell_ids, begin_Trajectory, begin_Step = ['begin_deg_traj_%d_time_%d' % (i, j) for j in deg_label_t for i in range(n_cell)], \
                                                 ...
                                                 [j for j in deg_label_t for i in range(n_cell)]  # first n_traj and then steps
        ...
        K = 2.5 * r
        n = 10
        params_unlab = {'a1': a1, 'b1': b1, 'a2': a2, 'b2': b2,
                        'a1_l': 0, 'b1_l': 0, 'a2_l': 0, 'b2_l': 0,
                        'K': K, 'n': n,
                        'be1': beta, 'ga1': gamma, 'et1': eta, 'de1': delta,
                        'be2': beta, 'ga2': gamma, 'et2': eta, 'de2': delta}
        params_lab = {'a1': 0, 'b1': 0, 'a2': 0, 'b2': 0,
                      'a1_l': a1, 'b1_l': b1, 'a2_l': a2, 'b2_l': b2,
                      'K': K, 'n': n,
                      'be1': beta, 'ga1': gamma, 'et1': eta, 'de1': delta,
                      'be2': beta, 'ga2': gamma, 'et2': eta, 'de2': delta}
        ...
        kin_5, kin_40, kin_200, kin_300, one_shot, deg_begin, deg_end = osc_diff_dup(n_species, trajs_C, model_lab, model_unlab, n_cell)
        ...
        # label time for kinetics experiment is 1 (actually it is one-shot experiment)
        kin_cell_ids, kin_Trajectory, kin_Step = ['kin_traj_%d_time_%f' % (i, j) for j in kin_Tl for i in
                                                  range(n_cell)], \
                                                 [i for j in kin_Tl for i in range(n_cell)], \
                                                 [j for j in kin_Tl for i in range(n_cell)]
        one_shot_cell_ids, one_shot_Trajectory, one_shot_Step = ['one_shot_traj_%d_time_%d' % (i, j) for j in kin_T_CP
                                                                 for i in range(n_cell)], \
                                                                [i for j in kin_T_CP for i in range(n_cell)], \
                                                                [j for j in kin_T_CP for i in
                                                                 range(n_cell)]  # first n_traj and then steps
        begin_cell_ids, begin_Trajectory, begin_Step = ['begin_deg_traj_%d_time_%d' % (i, j) for j in deg_label_t for i
                                                        in range(n_cell)], \
                                                       [i for j in deg_label_t for i in range(n_cell)], \
                                                       [j for j in deg_label_t for i in
                                                        range(n_cell)]  # first n_traj and then steps
        end_cell_ids, end_Trajectory, end_Step = ['end_deg_traj_%d_time_%d' % (i, j) for j in deg_label_t for i in
                                                  range(n_cell)], \
                                                 [i for j in deg_label_t for i in range(n_cell)], \
                                                 [j for j in deg_label_t for i in
                                                  range(n_cell)]  # first n_traj and then steps
        ...
        obs = pd.DataFrame({'cell_name': cell_ids,
                            'trajectory': Trajectory,
                            'time': Step,
                            'experiment_type': pd.Series(['kin_t_5', 'kin_t_40', 'kin_t_200', 'kin_t_300', 'one_shot', 'deg_beign',
                                 'deg_end']).repeat([kin_len, kin_len, kin_len, kin_len, one_shot_len, begin_len, end_len]).values})
        obs.set_index('cell_name', inplace=True)
    else:
