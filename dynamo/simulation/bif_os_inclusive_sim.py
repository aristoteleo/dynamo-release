...
def syn_deg_data(model_lab, model_unlab, n_trajs, i_t_cp, trajs_CP, Tl, n_cell, p):
    ...
    uu_deg, su_deg, ul_deg, sl_deg, pr_deg = np.zeros((len(Tl), len(n_trajs))), np.zeros((len(Tl), len(n_trajs))), \
                                             np.zeros((len(Tl), len(n_trajs))), np.zeros((len(Tl), len(n_trajs))), \
                                             np.zeros((len(Tl), len(n_trajs)))
    for i, t in enumerate(Tl):
        ...
        uu_deg[i], su_deg[i], ul_deg[i], sl_deg[i], pr_deg[i] = np.array(u), np.array(s), np.array(w), np.array(
            l), np.array(p)
...
def osc_diff_dup(n_species, trajs_C, model_lab, model_unlab, n_cell):
    ...
    t_lab = 1.0
    uu_kin, su_kin, ul_kin, sl_kin, pr_kin = np.zeros((len(T_CP), len(n_trajs))), np.zeros((len(T_CP), len(n_trajs))), \
                                             np.zeros((len(T_CP), len(n_trajs))), np.zeros((len(T_CP), len(n_trajs))), \
                                             np.zeros((len(T_CP), len(n_trajs)))
    for i, t_cp in enumerate(T_CP):
        ...
        uu_kin[i], su_kin[i], ul_kin[i], sl_kin[i], pr_kin[i] = np.array(u), np.array(s), np.array(w), np.array(
            l), np.array(p)

    Tl = np.array([0, 1, 2, 4, 8])
    # at the beginning
    deg_begin = syn_deg_data(model_lab, model_unlab, n_trajs, 0, trajs_CP, Tl, n_cell, p)
    # at the end
    deg_end = syn_deg_data(model_lab, model_unlab, n_trajs, -1, trajs_CP, Tl, n_cell, p)

    return uu_kin, su_kin, ul_kin, sl_kin, pr_kin, deg_begin, deg_end
