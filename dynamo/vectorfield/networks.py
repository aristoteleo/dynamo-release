...
def build_network_per_cluster(
    ...
    if full_reg_rank is None:
        full_reg_rank = rank_jacobian_genes(adata, groups=cluster, mode="full reg", abs=abs, return_df=True)
    if full_eff_rank is None:
        full_eff_rank = rank_jacobian_genes(adata, groups=cluster, mode="full eff", abs=abs, return_df=True)
