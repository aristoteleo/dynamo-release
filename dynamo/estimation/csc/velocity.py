...
class ss_estimation:
    ...
    def fit(
            ...
            if self.extyp.lower() == "deg":
                if np.all(self._exist_data("ul", "sl")):
                    ...
                    if self._exist_data("uu"):
                        ...
                        if cores == 1:
                            for i in tqdm(
                                range(n_genes), desc="estimating alpha"
                            ):
                                (
                                    alpha[i],
                                    alpha_b[i],
                ...
                if np.all(self._exist_data("ul", "uu", "su")):
                    ...
                    n_genes = self.data["uu"].shape[
                        0
                    ]  # self.get_n_genes(data=U)
                    ...
                    if np.all(self._exist_data("ul", "uu", "su")):
                        ...
                        if self._exist_data("ul") and self._exist_parameter(
                            ...
                            if one_shot_method in ["sci-fate", "sci_fate"]:
                                gamma, total0 = np.zeros(n_genes), np.zeros(
                                    n_genes
                                )
                                for i in tqdm(
                                    range(n_genes), desc="estimating gamma"
                                ):
                                    total = (
                                        self.data["uu"][i] + self.data["ul"][i]
                                    )
        ...
        if np.all(self._exist_data("p", "su")):
            ...
            n_genes = (
                len(ind_for_proteins) if ind_for_proteins is not None else 0
            )
