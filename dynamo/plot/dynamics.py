...
def dynamics(
    ...
    if group is not None:
        ...
        _group, grp_len = np.unique(adata.obs[group]), len(
    ...
    X_data, X_fit_data = [X_data[i] for i in gene_idx], [
            ...
            for i, gene_name in enumerate(valid_genes):
                if model == "moment":
                    ...
                    if has_splicing:
                        ...
                        Obs, Pred = np.vstack((uu, ul, su, sl)), np.vstack(
                            (u, w, s, l)
                        )
                    else:
                        layers = (
