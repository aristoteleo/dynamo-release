...
class TestUtils:
    ...
    def mkdirs_wrapper(path: Union[str, Path], abort=True):
        if os.path.exists(path):
            ...
            if abort:
                ...
                main_info(
                    str(path) + " : is a directory, continue using the old one"
                )
                return False
            else:
    ...
    def gen_zebrafish_test_data(basis="pca"):
        ...
        dyn.tl.reduceDimension(
            adata, basis=basis, n_pca_components=30, enforce=True
        )
        ...
        dyn.vf.rank_acceleration_genes(
            adata, groups="Cell_type", akey="acceleration", prefix_store="rank"
        )
        dyn.vf.rank_curvature_genes(
            adata, groups="Cell_type", ckey="curvature", prefix_store="rank"
        )
        dyn.vf.rank_velocity_genes(
            adata, groups="Cell_type", vkey="velocity_S", prefix_store="rank"
        )

        dyn.pp.top_pca_genes(adata, n_top_genes=100)
        top_pca_genes = adata.var.index[adata.var.top_pca_genes]
        dyn.vf.jacobian(
            adata, regulators=top_pca_genes, effectors=top_pca_genes
        )
        dyn.cleanup(adata)
